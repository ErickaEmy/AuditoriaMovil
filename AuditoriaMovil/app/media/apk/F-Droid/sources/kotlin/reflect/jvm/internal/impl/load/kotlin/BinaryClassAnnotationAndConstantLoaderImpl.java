...
public final class BinaryClassAnnotationAndConstantLoaderImpl extends AbstractBinaryClassAnnotationAndConstantLoader {
    ...
    public ConstantValue transformToUnsignedConstant(ConstantValue constant) {
        ...
        if (constant instanceof ByteValue) {
            uLongValue = new UByteValue(((Number) ((ByteValue) constant).getValue()).byteValue());
        } else if (constant instanceof ShortValue) {
            uLongValue = new UShortValue(((Number) ((ShortValue) constant).getValue()).shortValue());
        } else if (constant instanceof IntValue) {
            uLongValue = new UIntValue(((Number) ((IntValue) constant).getValue()).intValue());
        } else if (!(constant instanceof LongValue)) {
            return constant;
        } else {
            uLongValue = new ULongValue(((Number) ((LongValue) constant).getValue()).longValue());
        }
        return uLongValue;
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    @Override // kotlin.reflect.jvm.internal.impl.load.kotlin.AbstractBinaryClassAnnotationLoader
    public KotlinJvmBinaryClass.AnnotationArgumentVisitor loadAnnotation(final ClassId annotationClassId, final SourceElement source, final List result) {
        ...
        return new AbstractAnnotationArgumentVisitor() { // from class: kotlin.reflect.jvm.internal.impl.load.kotlin.BinaryClassAnnotationAndConstantLoaderImpl$loadAnnotation$1
            ...
            @Override // kotlin.reflect.jvm.internal.impl.load.kotlin.BinaryClassAnnotationAndConstantLoaderImpl.AbstractAnnotationArgumentVisitor
            public void visitArrayValue(Name name, ArrayList elements) {
                ...
                if (annotationParameterByName != null) {
                    ...
                    List compact = CollectionsKt.compact(elements);
                ...
                } else if (BinaryClassAnnotationAndConstantLoaderImpl.this.isImplicitRepeatableContainer(annotationClassId) && Intrinsics.areEqual(name.asString(), Action.VALUE_ATTRIBUTE)) {
                    ...
                    for (AnnotationValue annotationValue : arrayList) {
                        list.add((AnnotationDescriptor) annotationValue.getValue());
                    }
                }
            }
        ...
        public KotlinJvmBinaryClass.AnnotationArgumentVisitor visitAnnotation(final Name name, ClassId classId) {
            ...
            return new KotlinJvmBinaryClass.AnnotationArgumentVisitor(this, name, arrayList) { // from class: kotlin.reflect.jvm.internal.impl.load.kotlin.BinaryClassAnnotationAndConstantLoaderImpl$AbstractAnnotationArgumentVisitor$visitAnnotation$1
                ...
                public void visitEnd() {
                    ...
                    single = CollectionsKt___CollectionsKt.single((List) this.$list);
