...
public class TypeSubstitutor {
    ...
    */
    ...
    private TypeProjection unsafeSubstitute(TypeProjection typeProjection, TypeParameterDescriptor typeParameterDescriptor, int i) {
        ...
        if (!DynamicTypesKt.isDynamic(type) && !(type.unwrap() instanceof RawType)) {
            TypeProjection mo2227get = this.substitution.mo2227get(type);
            TypeProjection projectedTypeForConflictedTypeWithUnsafeVariance = mo2227get != null ? projectedTypeForConflictedTypeWithUnsafeVariance(type, mo2227get, typeParameterDescriptor, typeProjection) : null;
    ...
    private TypeProjection substituteCompoundType(TypeProjection typeProjection, int i) {
        KotlinType type = typeProjection.getType();
        Variance projectionKind = typeProjection.getProjectionKind();
        if (type.getConstructor().mo2222getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return typeProjection;
        }
    ...
    private List substituteTypeArguments(List list, List list2, int i) {
