...
public abstract class AbstractIncludeAction extends Action {
    ...
    private boolean checkAttributes(Attributes attributes) {
        ...
        if (i == 0) {
            format = String.format("One of \"%1$s\", \"%2$s\" or \"%3$s\" attributes must be set.", "file", RESOURCE_ATTR, URL_ATTR);
        } else if (i <= 1) {
            if (i == 1) {
                return true;
            }
            ...
            format = String.format("Only one of \"%1$s\", \"%2$s\" or \"%3$s\" attributes should be set.", "file", RESOURCE_ATTR, URL_ATTR);
        }
