...
public class InstallManagerService {
    ...
    private void queue(final String str, String str2, final App app, final Apk apk) {
        ...
        getMainObb(str, apk);
        getPatchObb(str, apk);
        Utils.runOffUiThread(new Supplier() { // from class: org.fdroid.fdroid.installer.InstallManagerService$$ExternalSyntheticLambda0
            @Override // androidx.core.util.Supplier
            public final Object get() {
                Pair lambda$queue$0;
                lambda$queue$0 = InstallManagerService.this.lambda$queue$0(apk);
                return lambda$queue$0;
            }
        }, new Consumer() { // from class: org.fdroid.fdroid.installer.InstallManagerService$$ExternalSyntheticLambda1
            @Override // androidx.core.util.Consumer
            public final void accept(Object obj) {
                InstallManagerService.this.lambda$queue$1(str, app, apk, (Pair) obj);
            }
        });
    }
    ...
    public /* synthetic */ void lambda$queue$1(String str, App app, Apk apk, Pair pair) {
        ...
        } else if (apkCacheState == ApkCache.ApkCacheState.CACHED) {
            Utils.debugLog(TAG, "skip download, we have it, straight to install " + str + " " + sanitizedFile);
            Uri parse = Uri.parse(str);
            onDownloadStarted(parse);
            onDownloadComplete(parse, sanitizedFile, app, apk);
        } else {
            Utils.debugLog(TAG, "delete and download again " + str + " " + sanitizedFile);
            Objects.requireNonNull(sanitizedFile);
            Utils.runOffUiThread(new Runnable() { // from class: org.fdroid.fdroid.installer.InstallManagerService$$ExternalSyntheticLambda2
                @Override // java.lang.Runnable
                public final void run() {
                    SanitizedFile.this.delete();
                }
            });
            DownloaderService.queue(this.context, str, app, apk);
        }
    ...
    public void onDownloadComplete(Uri uri, File file, App app, Apk apk) {
        ...
        Utils.debugLog(TAG, "download completed of " + uri + " to " + fromFile);
        this.appUpdateStatusManager.updateApk(uri2, AppUpdateStatusManager.Status.ReadyToInstall, null);
        App app2 = this.appUpdateStatusManager.getApp(uri2);
        Apk apk2 = this.appUpdateStatusManager.getApk(uri2);
        if (app2 != null && apk2 != null) {
            app = app2;
            apk = apk2;
        }
        if (app != null && apk != null) {
            registerInstallReceiver(uri2);
            InstallerService.install(this.context, fromFile, uri, app, apk);
            return;
        }
        Log.e(TAG, "Could not install " + uri2 + " because no app or apk available.");
    }
