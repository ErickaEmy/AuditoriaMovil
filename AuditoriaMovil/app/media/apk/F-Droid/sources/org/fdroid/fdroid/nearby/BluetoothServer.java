    ...
    private static class ClientConnection extends Thread {
        ...
        private Response respond(Map<String, String> map, String str) {
            ...
            if (file.isDirectory() && !replace.endsWith("/")) {
                ...
                return respond(map, replace + findIndexFileInDirectory);
            } else {
                Response serveFile = serveFile(replace, map, file, getMimeTypeForFile(replace));
                return serveFile != null ? serveFile : createResponse(NanoHTTPD.Response.Status.NOT_FOUND, NanoHTTPD.MIME_PLAINTEXT, "Error 404, file not found.");
            }
        }
