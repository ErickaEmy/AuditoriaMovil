...
public class CategoryController extends RecyclerView.ViewHolder {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public void bindModel(CategoryItem categoryItem, LiveData liveData) {
        ...
        if (icon != null && repository != null) {
            ...
            Glide.with((FragmentActivity) this.activity).load(Utils.getGlideModel(repository, icon)).apply((BaseRequestOptions) Utils.getAlwaysShowIconRequestOptions()).into(this.image);
        } else {
            ...
            if (categoryResource == 0) {
                ...
                Glide.with((FragmentActivity) this.activity).load(Integer.valueOf(categoryResource)).into(this.image);
            }
        }
    ...
    private static int getCategoryResource(Context context, String str, String str2, boolean z) {
        ...
        return resources.getIdentifier("category_" + replace, str2, context.getPackageName());
    }
    ...
    public static int getBackgroundColour(Context context, String str) {
        ...
        if (categoryResource > 0) {
            return ContextCompat.getColor(context, categoryResource);
        }
