        ...
        handler = new Handler(handlerThread.getLooper()) { // from class: org.fdroid.fdroid.nearby.BonjourManager.2
            ...
            public void handleMessage(Message message) {
                ...
                if (i == BonjourManager.STOP) {
                    handleStop();
                } else if (i == BonjourManager.VISIBLE) {
                    handleVisible(str, z);
                } else if (i == BonjourManager.VPN_CONFLICT) {
                    handleVpnConflict();
                } else if (i != BonjourManager.NOT_VISIBLE) {
                } else {
                    handleNotVisible();
                }
            }
            ...
            private void handleVisible(String str2, boolean z2) {
                ...
                hashMap.put(BonjourPeer.PATH, "/fdroid/repo");
                hashMap.put("name", str2);
                hashMap.put(BonjourPeer.FINGERPRINT, FDroidApp.repo.getFingerprint());
                if (z2) {
                    hashMap.put(BonjourPeer.TYPE, "fdroidrepos");
                    str3 = BonjourManager.HTTPS_SERVICE_TYPE;
                } else {
                    hashMap.put(BonjourPeer.TYPE, "fdroidrepo");
                    str3 = BonjourManager.HTTP_SERVICE_TYPE;
                }
                ServiceInfo create = ServiceInfo.create(str3, str2, FDroidApp.port, 0, 0, hashMap);
                if (!create.equals(BonjourManager.pairService)) {
                    try {
                        if (BonjourManager.pairService != null) {
                            BonjourManager.jmdns.unregisterService(BonjourManager.pairService);
                        }
                        BonjourManager.jmdns.registerService(create);
                        BonjourManager.pairService = create;
                    } catch (IOException e) {
                        e.printStackTrace();
                        BonjourManager.sendBroadcast(65535, e.getLocalizedMessage());
                        return;
