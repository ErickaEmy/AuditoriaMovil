...
public class DownloaderFactory extends org.fdroid.download.DownloaderFactory {
    ...
    public static final HttpManager HTTP_MANAGER = new HttpManager(Utils.getUserAgent(), FDroidApp.queryString, NetCipher.getProxy(), new DnsWithCache());
    ...
    protected Downloader create(Repository repository, List<Mirror> list, Uri uri, IndexFile indexFile, File file, Mirror mirror) throws IOException {
        ...
        String replace = uri.toString().replace(Utils.getRepoAddress(repository), "");
    ...
    private static List<Mirror> loadIpfsGateways(Preferences preferences) {
        ...
        for (String str : preferences.getActiveIpfsGateways()) {
            arrayList.add(new Mirror(str, null, true));
        }
        return arrayList;
    }
}
