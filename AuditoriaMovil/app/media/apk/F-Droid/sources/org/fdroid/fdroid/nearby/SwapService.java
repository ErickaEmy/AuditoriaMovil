...
public class SwapService extends Service {
    ...
    private final Preferences.ChangeListener httpsEnabledListener = new Preferences.ChangeListener() { // from class: org.fdroid.fdroid.nearby.SwapService$$ExternalSyntheticLambda3
    ...
    public void connectTo(Peer peer) {
        ...
        Repository createSwapRepo = FDroidApp.createSwapRepo(peer.getRepoAddress(), null);
        this.peerRepo = createSwapRepo;
        try {
            updateRepo(peer, createSwapRepo);
        } catch (Exception e) {
    ...
    public static IndexV1 getVerifiedRepoIndex(Repository repository, String str, File file) throws SigningException, IOException, NotFoundException, InterruptedException {
        ...
        return (IndexV1) new IndexV1Verifier(file, null, str).getStreamAndVerify(new Function1() { // from class: org.fdroid.fdroid.nearby.SwapService$$ExternalSyntheticLambda0
            ...
            public final Object invoke(Object obj) {
                IndexV1 lambda$getVerifiedRepoIndex$0;
                lambda$getVerifiedRepoIndex$0 = SwapService.lambda$getVerifiedRepoIndex$0((InputStream) obj);
                return lambda$getVerifiedRepoIndex$0;
            }
        }).getSecond();
    }
    ...
    private void updateRepo(Peer peer, Repository repository) throws IOException, InterruptedException, SigningException, NotFoundException {
        File createTempFile = File.createTempFile("swap", "", getApplicationContext().getCacheDir());
        try {
            try {
                this.index.postValue(getVerifiedRepoIndex(repository, peer.getFingerprint(), createTempFile));
                startPollingConnectedSwapRepo();
            } catch (NotFoundException unused) {
                Uri build = Uri.parse(repository.getAddress()).buildUpon().appendPath("index.jar").build();
                DownloaderFactory.INSTANCE.createWithTryFirstMirror(repository, build, FileV2.fromPath("/index.jar"), File.createTempFile("ignored-", "")).download();
    ...
    private void askServerToSwapWithUs(final Repository repository) {
        this.compositeDisposable.add(Completable.fromAction(new Action() { // from class: org.fdroid.fdroid.nearby.SwapService$$ExternalSyntheticLambda1
            @Override // io.reactivex.rxjava3.functions.Action
            public final void run() {
                SwapService.lambda$askServerToSwapWithUs$1(Repository.this);
            }
        }).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).onErrorComplete(new Predicate() { // from class: org.fdroid.fdroid.nearby.SwapService$$ExternalSyntheticLambda2
            @Override // io.reactivex.rxjava3.functions.Predicate
            public final boolean test(Object obj) {
                boolean lambda$askServerToSwapWithUs$2;
                lambda$askServerToSwapWithUs$2 = SwapService.this.lambda$askServerToSwapWithUs$2(repository, (Throwable) obj);
                return lambda$askServerToSwapWithUs$2;
            }
        }).subscribe());
    }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Utils.debugLog(TAG, "Asking server at " + repository.getAddress() + " to swap with us in return (by POSTing to \"/request-swap\" with repo \"" + uri + "\"): " + responseCode);
    ...
    public int onStartCommand(Intent intent, int i, int i2) {
        Intent intent2 = new Intent(this, SwapWorkflowActivity.class);
