...
public class ObfInstallerService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        Uri parse = Uri.parse(intent.getStringExtra(DownloaderService.EXTRA_CANONICAL_URL));
        ...
        if ("obf".equals(fileExtensionFromUrl)) {
            sendPostInstallAndCompleteIntents(parse, app, apk, new File(stringExtra));
        } else if (!"zip".equals(fileExtensionFromUrl)) {
            sendBroadcastInstall(Installer.ACTION_INSTALL_INTERRUPTED, parse, app, apk, "Only .obf and .zip files are supported: " + stringExtra);
        } else {
            try {
                File file = new File(stringExtra);
                ZipFile zipFile = new ZipFile(file);
                if (zipFile.size() < 1) {
                    sendBroadcastInstall(Installer.ACTION_INSTALL_INTERRUPTED, parse, app, apk, "Corrupt or empty ZIP file!");
                }
                ZipEntry nextElement = zipFile.entries().nextElement();
                File file2 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), nextElement.getName());
                FileUtils.copyInputStreamToFile(zipFile.getInputStream(nextElement), file2);
                file.delete();
                sendPostInstallAndCompleteIntents(parse, app, apk, file2);
            } catch (IOException e) {
                e.printStackTrace();
                sendBroadcastInstall(Installer.ACTION_INSTALL_INTERRUPTED, parse, app, apk, e.getMessage());
            }
        }
    }
