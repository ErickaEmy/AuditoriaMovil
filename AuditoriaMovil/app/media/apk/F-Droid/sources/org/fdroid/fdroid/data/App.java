...
public class App implements Comparable<App>, Parcelable {
    ...
    public void update(Context context, List<Apk> list, AppPrefs appPrefs) {
        ...
        Apk findSuggestedApk = findSuggestedApk(list, appPrefs);
        if (findSuggestedApk == null) {
            return;
        }
        if (this.autoInstallVersionCode <= 0) {
            long j = this.installedVersionCode;
            long j2 = findSuggestedApk.versionCode;
            if (j < j2) {
                this.autoInstallVersionCode = j2;
                this.autoInstallVersionName = findSuggestedApk.versionName;
            }
        }
        this.antiFeatures = findSuggestedApk.antiFeatures;
        this.antiFeatureReasons = findSuggestedApk.antiFeatureReasons;
        this.whatsNew = findSuggestedApk.whatsNew;
        this.isApk = findSuggestedApk.isApk();
    }
    ...
    public Uri getShareUri(Context context) {
        ...
        return Uri.parse(repository.getWebBaseUrl()).buildUpon().path(this.packageName).build();
    }
    ...
    public Apk getInstalledApk(Context context, List<Apk> list) {
        ...
        try {
            ...
            while (true) {
                ...
                apk = it.next();
                if (apk.versionCode == PackageInfoCompat.getLongVersionCode(packageInfo)) {
                    break;
                }
            }
            return apk == null ? new Apk(packageInfo) : apk;
    ...
    public boolean hasUpdates(List<Apk> list, AppPrefs appPrefs) {
        ...
        if (findSuggestedApk != null) {
            long j = this.installedVersionCode;
            return j > 0 && j < findSuggestedApk.versionCode;
        }
        return false;
    }
    ...
    public Apk findSuggestedApk(List<Apk> list, String str) {
        ...
        while (true) {
            ...
            apk = it.next();
            if (apk.compatible && (mostAppropriateSigner == null || mostAppropriateSigner.equals(apk.signer))) {
                if (Apk.RELEASE_CHANNEL_STABLE.equals(str)) {
                    if (apk.releaseChannels.contains(Apk.RELEASE_CHANNEL_STABLE)) {
                        break;
                    }
                } else if (apk.releaseChannels.contains(Apk.RELEASE_CHANNEL_STABLE) || apk.releaseChannels.contains(str)) {
                    break;
                }
            }
        }
        return (apk != null || list.size() <= 0) ? apk : list.get(0);
    }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:12:0x002e, code lost:
        if (r0 >= r5.getAttributeCount()) goto L29;
     */
    /* JADX WARN: Code restructure failed: missing block: B:14:0x003a, code lost:
        if (r5.getAttributeName(r0).equals("minSdkVersion") == false) goto L20;
     */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:25:0x0058, code lost:
        if (r5.getAttributeName(r0).equals("targetSdkVersion") == false) goto L24;
     */
    /* JADX WARN: Code restructure failed: missing block: B:26:0x005a, code lost:
        r1 = java.lang.Integer.parseInt(r5.getAttributeValue(r0));
     */
    /* JADX WARN: Code restructure failed: missing block: B:28:0x006d, code lost:
        if (r5.getAttributeName(r0).equals("maxSdkVersion") == false) goto L28;
     */
