...
public class NotificationHelper {
    ...
    public NotificationHelper(Context context) {
        ...
        from.createNotificationChannelsCompat(Arrays.asList(new NotificationChannelCompat.Builder(CHANNEL_INSTALLS, 2).setName(context.getString(R.string.notification_channel_installs_title)).setDescription(context.getString(R.string.notification_channel_installs_description)).build(), new NotificationChannelCompat.Builder(CHANNEL_SWAPS, 2).setName(context.getString(R.string.notification_channel_swaps_title)).setDescription(context.getString(R.string.notification_channel_swaps_description)).build(), new NotificationChannelCompat.Builder(CHANNEL_UPDATES, 2).setName(context.getString(R.string.notification_channel_updates_title)).setDescription(context.getString(R.string.notification_channel_updates_description)).build()));
        ...
        LocalBroadcastManager.getInstance(context).registerReceiver(new BroadcastReceiver() { // from class: org.fdroid.fdroid.NotificationHelper.1
            ...
            public void onReceive(Context context2, Intent intent) {
                ...
                AppUpdateStatusManager appUpdateStatusManager = AppUpdateStatusManager.getInstance(context2);
                ...
                switch (c) {
                    ...
                    case 1:
                        ...
                        return;
                    case 2:
                        AppUpdateStatusManager.AppUpdateStatus appUpdateStatus3 = appUpdateStatusManager.get(intent.getStringExtra(DownloaderService.EXTRA_CANONICAL_URL));
                        NotificationHelper.this.updateStatusLists();
                        if (appUpdateStatus3 != null) {
                            NotificationHelper.this.createNotification(appUpdateStatus3);
                        }
    ...
    public void createNotification(AppUpdateStatusManager.AppUpdateStatus appUpdateStatus) {
        ...
        if (shouldIgnoreEntry(appUpdateStatus)) {
        ...
        } else if (!this.notificationManager.areNotificationsEnabled() || Preferences.get().hideAllNotifications()) {
        } else {
            if (appUpdateStatus.status == AppUpdateStatusManager.Status.Installed) {
            ...
            } else if (useStackedNotifications()) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: org.fdroid.fdroid.NotificationHelper$3  reason: invalid class name */
    /* loaded from: classes2.dex */
    ...
    private Notification createInstalledSummaryNotification(ArrayList<AppUpdateStatusManager.AppUpdateStatus> arrayList) {
        ...
        NotificationCompat.Builder visibility = new NotificationCompat.Builder(this.context, CHANNEL_INSTALLS).setAutoCancel(!useStackedNotifications()).setSmallIcon(R.drawable.ic_notification).setColor(ContextCompat.getColor(this.context, R.color.fdroid_blue)).setContentTitle(quantityString).setContentText(sb).setContentIntent(PendingIntent.getActivity(this.context, 0, new Intent(this.context, MainActivity.class), 201326592)).setLocalOnly(true).setVisibility(-1);
        if (useStackedNotifications()) {
    ...
    public Point getLargeIconSize() {
        return new Point(this.context.getResources().getDimensionPixelSize(17104901), this.context.getResources().getDimensionPixelSize(17104902));
    }
    ...
    private void loadLargeIconForEntry(AppUpdateStatusManager.AppUpdateStatus appUpdateStatus, final NotificationCompat.Builder builder, final int i, final String str) {
        Context context = this.context;
        App app = appUpdateStatus.app;
        ((RequestBuilder) ((RequestBuilder) App.loadBitmapWithGlide(context, app.repoId, app.iconFile).fallback(R.drawable.ic_notification_download)).error(R.drawable.ic_notification_download)).into((RequestBuilder) new CustomTarget() { // from class: org.fdroid.fdroid.NotificationHelper.2
