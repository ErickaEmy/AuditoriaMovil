...
class HT {
    ...
    public byte[] sign(byte[] bArr, long j, int i) {
        ...
        while (true) {
            ...
            if (i4 >= sPHINCSPlusEngine.D) {
                break;
            }
            int i5 = (int) (((1 << i2) - 1) & j2);
    ...
    public boolean verify(byte[] bArr, SIG_XMSS[] sig_xmssArr, byte[] bArr2, long j, int i, byte[] bArr3) {
        ...
        while (true) {
            ...
            adrs.setTreeAddress(j);
            xmss_pkFromSig = xmss_pkFromSig((int) (((1 << i2) - 1) & j), sig_xmss2, xmss_pkFromSig, bArr2, adrs);
            i3++;
        }
    }
