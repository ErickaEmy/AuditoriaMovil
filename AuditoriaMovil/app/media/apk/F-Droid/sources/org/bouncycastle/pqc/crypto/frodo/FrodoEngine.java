...
public class FrodoEngine {
    ...
    private byte[] ctselect(byte[] bArr, byte[] bArr2, short s) {
        ...
        for (int i = 0; i < bArr.length; i++) {
            bArr3[i] = (byte) (((~s) & bArr[i] & GF2Field.MASK) | (bArr2[i] & s & GF2Field.MASK));
        }
        return bArr3;
    }
    ...
    private short[] unpack(byte[] bArr, int i, int i2) {
        ...
        while (s < i3 && (s2 < bArr.length || (s2 == bArr.length && b > 0))) {
            ...
            while (true) {
                ...
                sArr[s] = (short) (((sArr[s] & 65535) + ((((byte) ((((b2 & 255) >>> ((b & 255) - min)) & (s3 & 65535)) & GF2Field.MASK)) & 255) << ((this.D - (b3 & 255)) - min))) & 65535);
                b3 = (byte) (b3 + min);
                byte b4 = (byte) (b - min);
