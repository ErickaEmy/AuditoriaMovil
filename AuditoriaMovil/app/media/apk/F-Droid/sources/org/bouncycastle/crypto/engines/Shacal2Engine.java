...
public class Shacal2Engine implements BlockCipher {
    ...
    private void byteBlockToInts(byte[] bArr, int[] iArr, int i, int i2) {
        while (i2 < 8) {
            int i3 = ((bArr[i + 1] & GF2Field.MASK) << 16) | ((bArr[i] & GF2Field.MASK) << 24);
            int i4 = i + 3;
            i += 4;
            iArr[i2] = ((bArr[i + 2] & GF2Field.MASK) << 8) | i3 | (bArr[i4] & GF2Field.MASK);
            i2++;
        }
    }
    ...
    private void bytes2ints(byte[] bArr, int[] iArr, int i, int i2) {
        while (i2 < bArr.length / 4) {
            ...
            int i4 = i + 3;
            i += 4;
            iArr[i2] = i3 | ((bArr[i + 2] & GF2Field.MASK) << 8) | (bArr[i4] & GF2Field.MASK);
            i2++;
        }
    }
