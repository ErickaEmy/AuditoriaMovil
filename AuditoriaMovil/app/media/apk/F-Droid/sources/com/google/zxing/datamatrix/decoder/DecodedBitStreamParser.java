...
abstract class DecodedBitStreamParser {
    ...
    static {
        ...
        TEXT_SHIFT3_SET_CHARS = new char[]{'`', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', Matrix.MATRIX_TYPE_RANDOM_LT, 'M', 'N', 'O', 'P', 'Q', Matrix.MATRIX_TYPE_RANDOM_REGULAR, 'S', 'T', Matrix.MATRIX_TYPE_RANDOM_UT, 'V', 'W', 'X', 'Y', Matrix.MATRIX_TYPE_ZERO, CoreConstants.CURLY_LEFT, '|', CoreConstants.CURLY_RIGHT, '~', 127};
    }
                ...
                if (i != 0) {
                    if (i != 1) {
                        if (i == 2) {
                            ...
                            if (z) {
                                sb.append((char) (i3 + BERTags.FLAGS));
                                z = false;
                                i = 0;
                            } else {
    ...
    private static void decodeBase256Segment(BitSource bitSource, StringBuilder sb, Collection collection) {
    ...
    private static int unrandomize255State(int i, int i2) {
        int i3 = i - (((i2 * 149) % GF2Field.MASK) + 1);
