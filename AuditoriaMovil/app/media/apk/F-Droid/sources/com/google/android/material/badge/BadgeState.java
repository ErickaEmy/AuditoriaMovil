...
public final class BadgeState {
    ...
    public BadgeState(Context context, int i, int i2, int i3, State state) {
        ...
        state2.alpha = state.alpha == -2 ? GF2Field.MASK : state.alpha;
        if (state.number == -2) {
        ...
        if (state.maxCharacterCount == -2) {
            i4 = generateTypedArray.getInt(R$styleable.Badge_maxCharacterCount, -2);
        } else {
            i4 = state.maxCharacterCount;
        }
        state2.maxCharacterCount = i4;
        if (state.maxNumber == -2) {
            i5 = generateTypedArray.getInt(R$styleable.Badge_maxNumber, -2);
        } else {
            i5 = state.maxNumber;
        }
        state2.maxNumber = i5;
