...
public class ServiceInfoResolver extends DNSResolverTask {
    ...
    protected DNSOutgoing addAnswers(DNSOutgoing dNSOutgoing) {
        if (!this._info.hasData()) {
            ...
            dNSOutgoing = addAnswer(addAnswer(dNSOutgoing, (DNSRecord) cache.getDNSEntry(qualifiedName, dNSRecordType, dNSRecordClass), currentTimeMillis), (DNSRecord) getDns().getCache().getDNSEntry(this._info.getQualifiedName(), DNSRecordType.TYPE_TXT, dNSRecordClass), currentTimeMillis);
            if (this._info.getServer().length() > 0) {
                for (DNSEntry dNSEntry : getDns().getCache().getDNSEntryList(this._info.getServer(), DNSRecordType.TYPE_A, dNSRecordClass)) {
